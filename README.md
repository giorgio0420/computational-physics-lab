...
2026 pass that got it running again, tested it, and wrote down what had to change. That
second pass was done with the help of an AI assistant. The physics, the algorithms and
the original report are mine; the porting, the debugging and the packaging were done
together with it, and `gas.c` was rewritten from scratch in the process.
...
![Lattice gas at three densities, empty pockets coloured by cluster](figures/gas.gif)

*Generated by `scripts/animate_gas.py`. Empty sites are coloured by which connected pocket
of empty space they belong to, wrapping across the periodic boundary; particles are dark.*
...
scripts/animate_gas.py builds figures/gas.gif from the lattice gas
